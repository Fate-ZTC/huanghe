...
<body>
    ...
    <ul class="placeul">
    <li><a href="#">消防巡查</a></li>
    <li><a href="<%=path %>/firePatrolUserList">巡查人员管理</a></li>
    </ul>
    </div>
    ...
    <div class="tools">
        ...
        <li onclick="bulkDelete('<%=path %>/firePatrolUser_delete','0');"><span><img src="<%=path %>/page/images/t03.png" /></span>删除</li>
        </sec:authorize>
        <li><span>${msg }</span></li>
        </ul>
    ...
    <table class="tablelist">
        ...
        <tbody>
        <c:forEach items="${firePatrolUserPage.list}" var="d">
        <tr>
   	...
   	<!-- 分页开始 -->
    <div class="pagin">
    	<div class="message">共<i class="blue">${firePatrolUserPage.allRow}</i>条记录，当前显示第&nbsp;<i class="blue"> ${firePatrolUserPage.currentPage}/${firePatrolUserPage.totalPage }&nbsp;</i>页</div>
        <ul class="paginList">
			<c:forEach items="${firePatrolUserPage.fristPage}" var="f">
				<li class="paginItem"><a href="javascript:void(0);" rel="${f}">${f}</a></li>
			</c:forEach>
			<li class="paginItem current"><a href="javascript:void(0);">${firePatrolUserPage.currentPage}</a></li>
			<c:forEach items="${firePatrolUserPage.laPage}" var="l">
