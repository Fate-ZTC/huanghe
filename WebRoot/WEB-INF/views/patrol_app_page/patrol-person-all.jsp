		...
		<div class="box f14">
			<div class="patrol-query-tab-box">
				...
				<div class="date-select-result">
					<span>2018</span>
					<div class="delete-date-result" onclick=""></div>
				</div>
					...
					for (var i = 0; i < showData.length; i++) {
						...
						if (this_li_id === undefined) {
							...
							itemAppend += '<li id="' + titleDate + '">' +
									...
									'<li class="patrol-situation-person-time">巡更区域:' + regionName + '</li>' +
									'<li class="patrol-situation-person">巡更开始时间：' + starttimeAll + '</li>' +
									'<li class="patrol-situation-person">巡更结束时间：' + endtimeAll + '</li>' +
									'</ul></div>';
//								'</li>';
							//判断是否有异常信息
							if (status === 1) {
...
//									'<div class="patrol-situation-abnormal-more">更多</div>';
			...
			calendar.init({
				...
				'onSubmit':function(){/*确认时触发事件*/
					...
					page = 1;
					clearDate();
					ajaxRequestDate(type,dateType,page,pageSize,jobNum,start,end);
				},
			 ...
			 * 通过ajax进行获取数据
			 */
			var ajaxRequestDate = function (type,dateType,page,pageSize,jobNum,startTime,endTime) {
				//下面进行参数拼装
				var param = '';
				if(dateType == 3) {
					param = 'type=' + type + "&dateType=" + dateType + '&jobNum=' + jobNum + '&page='
							+ page + '&pageSize=' + pageSize + '&startTime=' + startTime + '&endTime=' + endTime;
				}else {
					param = 'type=' + type + '&jobNum='+ jobNum + "&dateType=" + dateType + '&page='
							+ page + '&pageSize=' + pageSize;
				}

				$.ajax({
					url: '<%=basePath%>statistics/getPatrolPersonData?' + param,
					type: 'get',
...
//									'<div class="patrol-situation-abnormal-more">更多</div>';
			...
			$(".delete-date-result").click(function () {
				...
				dateType = 1;
				clearDate();
				ajaxRequestDate(0,1,page,pageSize,jobNum,startDate,endDate);
				activeChange();
			});
