    ...
    ms.classes.Widget = function (el, settings, inherit) {
        ...
        */
        that.show = function (prevAnim, prevFocus) {
                    ...
                    '<div class="dw-persp">' +
                        ...
                        '<div' + (isModal ? ' role="dialog" tabindex="-1"' : '') + ' class="dw' + (s.rtl ? ' dw-rtl' : ' dw-ltr') + '">' + // Popup
                            ...
                            '<div class="dwwr">' + // Popup content
                                ...
                                (s.headerText ? '<div class="dwv">' + s.headerText + '</div>' : '');
            ...
            html +='<div class="dwcc">'; // Wheel group container
            ...
            $.each(s.wheels, function (i, wg) { // Wheel groups
                ...
                $.each(wg, function (j, w) { // Wheels
                    ...
                    html += generateWheelItems(l) +
                        ...
                        (s.selectedLineHeight ? ' style="height:' + itemHeight + 'px;margin-top:-' + (itemHeight / 2 + (s.selectedLineBorder || 0)) + 'px;"' : '') + '></div><div class="dwwol"' +
                        (s.selectedLineHeight ? ' style="height:' + itemHeight + 'px;margin-top:-' + (itemHeight / 2 + (s.selectedLineBorder || 0)) + 'px;"' : '') + '></div><div class="dwwol"' +
                ...
                for (k = 0; k < 3; k++) {
                    if (k == o.y) {
                        ...
                        for (i = 0; i < 12; i++) {
                            ...
                            keys.push(i);
                            values.push(str.match(/MM/) ? str.replace(/MM/, '<span class="dw-mon">' + s.labels[1] + '</span>') : str.replace(/M/, '<span class="dw-mon">' + s.labels[1] + '</span>'));
                        }
                        // console.log(s)
                            ...
                            if (i == 'h') {
                                if (regen) {
                                    $('.dw-li', t).each(function () {
                                        var that = $(this),
                                        n = that.data('val');
                                        var ordh = s.timeWheels.match(/HH/) ? s.timeWheels.replace(/hhHH|hh|HH/,'hhHH') : s.timeWheels.replace(/hH|h|H/,'hH');
                                        str = ordh.replace(/[mi]/gi, '').replace(/hh/, (n < 10 ? '0' + n : n) ).replace(/h/, n );

                                        $('.dw-i', that).html(str.match(/HH/) ? str.replace(/HH/, '<span class="dw-hou">时</span>') : str.replace(/H/, '<span class="dw-hou">时</span>'));
                            ...
                            if (i == 'i') {
                                if (regen) {
                                    $('.dw-li', t).each(function () {
                                        var that = $(this),
                                        h = that.data('val');
                                        var ordh = s.timeWheels.match(/ii/) ? s.timeWheels.replace(/iiII|ii|II/,  'iiII') : s.timeWheels.replace(/iI|i|I/,'iI');
                                        str = ordh.replace(/[mh]/gi, '').replace(/ii/, (h < 10 ? '0' + h : h) ).replace(/i/, h );

                                        $('.dw-i', that).html(str.match(/II/) ? str.replace(/II/, '<span class="dw-hou">分</span>') : str.replace(/I/, '<span class="dw-hou">分</span>'));
                                    });
                                }
                            }
                            if (minprop && mind) {
