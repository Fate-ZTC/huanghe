...
public class JPushClientExample {
     ...
     * @param map，封装APP接收消息后动作需要的参数，主要包括： 1.type,消息类型，目前有： 1.系统通知；2.版本升级；3.个推类型消息; 4.座位分配成功通知；5.签到通知提示；6、座位续约通知；7.预约自动结束通知；8.排队预约失效通知
     *                                    2.id,根据不同类型
     */
        ...
        return PushPayload.newBuilder()
                ...
                .setNotification(Notification.newBuilder()
                        .addPlatformNotification(IosNotification.newBuilder()
                                ...
                                .addExtra("id", (String) map.get("id"))
                                .addExtra("content", (String) map.get("content"))
                                .addExtra("userCode", map.get("userCode"))
                                .addExtra("userName", map.get("userName"))
                                .addExtra("lon", map.get("lon"))
                                .addExtra("lat", map.get("lat"))
                                .build())
                        .addPlatformNotification(AndroidNotification.newBuilder()
                                .setAlert(msgContent)
                                .setTitle(msgTitle)
